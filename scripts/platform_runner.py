...
def ensure_pages_project(project_name: str, workdir: Path) -> None:
    command = [
        "npx",
        "wrangler@latest",
        "pages",
...
def set_pages_secret(project_name: str, secret_name: str, secret_value: str, workdir: Path) -> None:
    if not secret_value:
        return
    command = [
        "npx",
...
def deploy_pages(project_name: str, output_dir: Path, workdir: Path) -> None:
    command = [
        "npx",
        "wrangler@latest",
        "pages",
...
def run_scene_wiki_build(job: dict, workdir: Path, custom_domain: str) -> Path:
    ...
    subprocess.run(command, cwd=workdir, env=command_env(job, title, custom_domain), check=True)
    return output_dir
...
def handle_job(base_url: str, job: dict, workdir: Path) -> None:
    ...
    try:
        ...
        output_dir = run_scene_wiki_build(job, workdir, custom_domain)

        heartbeat(base_url, job["id"], "Ensuring Cloudflare Pages project exists.")
        ensure_pages_project(project_name, workdir)
        ...
        for secret_name in ("GOOGLE_API_KEY", "GEMINI_API_KEY"):
          ...
          if secret_value:
              ...
              set_pages_secret(project_name, secret_name, secret_value, workdir)

        heartbeat(base_url, job["id"], "Deploying site bundle to Cloudflare Pages.")
        deploy_pages(project_name, output_dir, workdir)
